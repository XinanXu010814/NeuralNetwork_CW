...
class MultiLayerNetwork(object):
    ...
    def __init__(self, input_dim, neurons, activations):
        ...
        #######################################################################
        self._layers = []
        neurons_ = [input_dim] + neurons
        for i in range(len(neurons)):
            self._layers.append(LinearLayer(neurons_[i], neurons_[i+1]))
...
class Trainer(object):
    ...
    def train(self, input_dataset, target_dataset):
        ...
        for _ in range(self.nb_epoch):
            ...
            batch_num = len(input_dataset) // self.batch_size
            minibatches_input = np.array_split(input_dataset, batch_num)
            minibatches_target = np.array_split(target_dataset, batch_num)
            for batch_index in range(batch_num):
                y_pred = self.network.forward(minibatches_input[batch_index])
                self._loss_layer.forward(y_pred, minibatches_target[batch_index])
...
class Preprocessor(object):
    ...
    def __init__(self, data):
        ...
        # self._data = data
        # if not np.isclose(np.max(data), self._min).all():
        #     self._scale = 1 / (np.max(data) - self._min)
        # else:
        #     # if all values in array are the same, data do not 
        #     # provide any useful information
        #     self._scale = self._min
    ...
    def apply(self, data):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        # if not np.isclose(self._min, np.min(data)) or not np.isclose(np.max(data), np.max(self._data)):
...
def example_main():
    ...
    # in_mat = np.matrix([[-1,2,-3], [5,-6,7]])
    # net = SigmoidLayer()
    # print(net.forward(in_mat))
    # print(net.backward(np.matrix([[1,2,-3], [-5,-6,7]])))
    ...
    print(np.sum(np.abs(prep_input.revert(prep_input.apply(x_train)) - x_train)))
